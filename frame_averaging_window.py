...
class RadarConfig:
    ...
    # --- Denoising and Smoothing Parameters ---
    # 1. Median Filter: Removes "salt and pepper" noise. Use an odd number (3 or 5).
    noise_filter_size: int = 3
    
    # 2. Frame Averaging: Smooths the animation by averaging frames.
    # A larger window gives more smoothing but introduces a slight "ghosting" trail.
    # A value of 3 or 4 is a good starting point.
    frame_averaging_window: int = 4
...
class AzimuthProcessor:
    ...
    def process_frame(self, frame):
        ...
        # Step 5: Apply the 2D Median Filter
...
def main():
    ...
    # Process all frames first to get individual heatmaps
    individual_heatmaps = [processor.process_frame(frame) for frame in organized_data]
    
    # --- NEW: Apply Frame Averaging ---
    print("Applying frame averaging...")
    averaged_heatmaps = []
    window_size = config.frame_averaging_window
    for i in range(len(individual_heatmaps)):
        # Get the slice of frames to average
        start_index = max(0, i - window_size + 1)
        frames_to_average = individual_heatmaps[start_index : i+1]
        
        # Average them along the new 'frame' axis
        averaged_frame = np.mean(np.array(frames_to_average), axis=0)
        averaged_heatmaps.append(averaged_frame)
    # ------------------------------------

    print("\nGenerating animation...")
    processor.animate_heatmaps(averaged_heatmaps)
